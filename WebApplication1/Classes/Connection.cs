...
namespace WebApplication1.Classes
{
    class Connection
    {
        ...
        public Connection()
        {
            try
            {
                cn = new SqlConnection(@"Data Source=DESKTOP-KNHT7RE\SQLEXPRESS;Initial Catalog=ECanteen;Integrated Security=True");
                cn.Open();
            }
            catch (Exception ex)
            {
        ...
        public SqlDataReader GetSqlReaderQuery(string query)
        {
            try
            {
                SqlCommand cmd = new SqlCommand(query, cn);
